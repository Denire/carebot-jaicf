...
val MainScenario = Scenario {
    handle<BotRequestHook> {
        println("BotRequestHook: " + context.dialogContext.currentContext)
        if (context.dialogContext.currentContext == "/") {
            println("Initial request, going to /Start")
//                hook.reactions.go("/Start")
            context.dialogContext.currentContext = "/Start"
        }
    }

    handle<BeforeProcessHook> {
        println("BeforeProcessHook: " + context.dialogContext.currentContext)
    }

    handle<AfterProcessHook> {
        println("AfterProcessHook: " + context.dialogContext.currentContext)
    }
    ...
    state("Start") {
        ...
        state("WhatDoYouDo") {
            ...
            action {
                reactions.say("Я помогаю общаться с незнакомыми людьми, выходить из трудных ситуаций. B еще я умею отвечать на каверзные вопросы.")
                reactions.sayRandom("вот спроси меня, что такое котлеты",
                    "вот попроси меня сыграть в магазин",
                    "вот попроси меня сыграть во вредные советы")
                reactions.go("/")
            }
        }
        ...
        state("Obsession") {
            ...
            action {
                reactions.sayRandom("Марк, давай поговорим об этом позже.",
                    "Об этом я тебе еще как-нибудь расскажу.")
    ...
    state("Youtellme") {
        ...
        state("1") {
            action {
                reactions.sayRandom("Должно быть, так оно и есть.",
    ...
    state("Ask") {
        action {
            val question =
                randSelect(mutableListOf("Food", "Supermarket", "BadAdv", "Friend", "Art", "Music", "Cop"))
        ...
        state("Cop") {
            ...
            state("Next") {
                action {
                    reactions.say("Сперва, ты должен успокоиться. Не волнуйся, ты ничего плохого не сделал, наверное начал разговаривать сам с собой и это показалось ему странно.")
                    reactions.go("1")
                }
                state("1") {
        ...
        state("BadAdv") {
            action {
                reactions.sayRandom("Давай придумаем вредный совет!",
                    "Давай придумаем совет, только очень вредный!")
    ...
    state("Reward2") {
        action {
            ...
            reactions.telegram?.image("https://upload.wikimedia.org/wikipedia/commons/thumb/9/90/Austin_Texas_Lake_Front.jpg/800px-Austin_Texas_Lake_Front.jpg",
                "Texas")
        }
        fallback {
            reactions.go("/End")
